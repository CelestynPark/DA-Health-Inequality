...
def plot_scatter(df, output_path):
    ...
    sns.regplot(
        data=df,
        x="Unemployment raw value",
        y="Premature Death raw value",
        scatter_kws={"alpha": 0.6},
        line_kws={"color": "red"}
    )
    plt.title("Unemployment vs Premature Death by County")
...
if __name__ == "__main__":
    ...
    output_path = os.path.join("output", "figures", "unemployment_vs_premature_death_scatter.png")

    df = load_data(data_path)
    df_selected = select_data(df)
    plot_scatter(df_selected, output_path)

    print(f"[INFO] Scatter plot saved to: {output_path}")